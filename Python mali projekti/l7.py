...
def russianRoulete(population): # selekcija
	...
	izbor = np.random.choice(range(0, br_populacije), size =round(br_populacije/5), p= niz_verovatnoca, replace = False)
	for i in range(round(br_populacije/5)):
...
def ukrstanje(population):
	...
	newPopulation = []
	wall = random.randrange(2, len(fajl) - 1)
